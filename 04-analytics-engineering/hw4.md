...
```
select pickup_zone, sum(revenue_monthly_total_amount) as total_revenue_2020 from prod.fct_monthly_zone_revenue where service_type='Green' and year(revenue_month) = 2020 group by pickup_zone order by total_revenue_2020 desc limit 1;
```
...
```
D select count(*) from prod.raw_fhv_tripdata;
┌─────────────────┐
│  count_star()   │
│      int64      │
├─────────────────┤
